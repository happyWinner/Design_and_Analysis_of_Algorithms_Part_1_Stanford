...
Consider the first, middle, and final elements of the given array. (If the array has odd length it should be clear what the "middle" element is; for an array with even length 2k, use the kth element as the "middle" element. So for the array 4 5 6 7, the "middle" element is the second one ---- 5 and not 6!)
...
unsigned long QuickSort(vector<int>&, int, int, int);
int ChoosePivot(vector<int>&, int, int, int);
...
int main()
{
	...
	cout << "Problem 1: " << QuickSort(array1, 0, length - 1, 1) << endl;
	cout << "Problem 2: " << QuickSort(array2, 0, length - 1, 2) << endl;
	cout << "Problem 3: " << QuickSort(array3, 0, length - 1, 3) << endl;

	return 0;
}

unsigned long QuickSort(vector<int>& array, int start, int end, int pivotSelection)
{
	if(start >= end)
		return 0;

	unsigned long numberOfComparisons = end - start;
	int pivot = ChoosePivot(array, start, end, pivotSelection);
	swap(array.at(start), array.at(pivot));

	int i = start + 1;
	for(int j = start + 1; j <= end; j++)
	{
		if(array.at(j) < array.at(start))
		{
			swap(array.at(j), array.at(i));
			i++;
		}
	}
	swap(array.at(start), array.at(i - 1));

	numberOfComparisons += QuickSort(array, start, i - 2, pivotSelection);
	numberOfComparisons += QuickSort(array, i, end, pivotSelection);

	return numberOfComparisons;
}


int ChoosePivot(vector<int>& array, int start, int end, int pivotSelection)
{
	int pivot;
	...
	case 3:
		...
		if(array.at(start) > array.at(middle) && array.at(start) < array.at(end) || array.at(start) < array.at(middle) && array.at(start) > array.at(end))
			pivot = start;
		else if(array.at(middle) > array.at(start) && array.at(middle) < array.at(end) || array.at(middle) < array.at(start) && array.at(middle) > array.at(end))
			pivot = middle;
		else
			pivot = end;
		break;
